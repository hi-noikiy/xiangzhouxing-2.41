...
Anim.Page({
  ...
  onLoad: function (options) {
    this.data.vehicleTypeCode = options.vehicleTypeCode ? String(options.vehicleTypeCode) : '3';
    this.setData({
      'formData.vehicleTypeCode': options.vehicleTypeCode ? String(options.vehicleTypeCode) : '3'
    })
    let id = options.id;
  ...
  handleSubmit(e) {
    ...
    if (this.data.vehicleTypeCode=='undefined') {
      ...
      obj = {
        vehicleTypeCode: this.data.formData.vehicleTypeCode,
    ...
    editApi.saveOrUpdate(params).then(res => {
      wx.hideLoading()
      wx.navigateTo({
        url: '/pages/car-code/bus-code/car-success/index?id=' + res.checkPointRegisterId + "&identityNo=" + this.data.userInfo.uid + "&isRail=true",
      })
  ...
  handleChange(e) {
    ...
    console.log(this.data.formData.vehicleTypeCode)
  },
  ...
  getDetail(id) {
    ...
    detial.getTrafficgateInfo({
      ...
      result.vehicleTypeCode = String(result.vehicleTypeCode);
